...
function getStatusCount(
  ...
  return values.find((item) => item._id === status)?.count || 0;
}
...
export default async function AdminAnalyticsPage({
  ...
  const orderStatuses = [
    { name: "Pending", value: getStatusCount(orderStatusBreakdown, "pending") },
    { name: "Confirmed", value: getStatusCount(orderStatusBreakdown, "confirmed") },
    { name: "Processing", value: getStatusCount(orderStatusBreakdown, "processing") },
    { name: "Shipped", value: getStatusCount(orderStatusBreakdown, "shipped") },
    { name: "Delivered", value: getStatusCount(orderStatusBreakdown, "delivered") },
    { name: "Cancelled", value: getStatusCount(orderStatusBreakdown, "cancelled") },
  ];

  const paidOrders = getStatusCount(paymentStatusBreakdown, "paid");
  const pendingPayments = getStatusCount(paymentStatusBreakdown, "pending");
  const failedPayments = getStatusCount(paymentStatusBreakdown, "failed");
  ...
  const isDaily = activePeriod === "7d" || activePeriod === "30d";
  const labelStep = isDaily && revenueTrend.length > 10 ? 5 : 1;
      ...
      <section className="mt-6 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="mt-8 flex h-48 items-end gap-1 sm:gap-2">
          {revenueTrend.map((point, index) => {
            const height =
              maxRevenue > 0 ? Math.max((point.revenue / maxRevenue) * 100, 3) : 3;
            ...
            return (
              ...
              >
                <div className="flex w-full flex-1 items-end justify-center">
                  <div
                    title={`${point.label}: Rs. ${formatPrice(point.revenue)}`}
                    className={`w-full max-w-[28px] rounded-t-md ${
                      point.revenue > 0 ? "bg-indigo-600" : "bg-neutral-200"
                    }`}
                    style={{ height: `${height}%` }}
                  />
                </div>
