...
export function parsePeriod(
  ...
  if (
    value &&
    (validPeriods as string[]).includes(value)
  ) {
...
function zeroFillTrend(
  period: AnalyticsPeriod,
  raw: { _id: string; revenue: number }[]
): RevenueTrendPoint[] {
  const revenueByKey = new Map(
    raw.map((item) => [item._id, item.revenue])
  );
...
export async function getAdminStatistics(
  ...
  const revenueMatch = {
    ...
    orderStatus: { $ne: "cancelled" as const },
    ...dateFilter,
  };
  ...
  const [
    ...
    Order.find(dateFilter)
      ...
      .select(
        "user orderNumber total orderStatus paymentStatus createdAt"
      )
      .sort({
        createdAt: -1,
      })
  ...
  const pendingRevenue =
    pendingRevenueResult[0]?.total || 0;

  const avgOrderValue =
    paidOrderCount > 0
      ? Math.round((totalRevenue / paidOrderCount) * 100) /
        100
      : 0;
